...
- Added FUT-006 sanitized audit history in a versioned local SQLite database with GitHub-host-and-account hashed identity, report-to-configuration account binding, semantic run and finding keys, coverage-aware resolution, and new, persistent, resolved, and regressed transitions.
...
- Updated `audit` to record sanitized local history after GitHub reads while preserving its JSON and Markdown output contracts and exit-code precedence. Partial or inconclusive audits are recorded but cannot resolve findings outside conclusive repository-and-check results. A history write failure preserves the audit report, emits a sanitized stderr error, and exits `2`.
...
- History state rejects Git worktrees, symbolic links including broken links, junctions, irregular database or backup targets, out-of-order runs, duplicate finding identities, corrupt databases, and schemas newer than the application.
