...
- Verification: Synthetic contracts cover complete settings and security evidence plus supported, inaccessible, not-applicable, unavailable-by-plan, unverified, unsupported, archived, plan-restricted, and permission-failure behavior. The locked suite passes Ruff, formatting, strict Pyright, and 134 pytest tests with 92.89% coverage. The expanded private pilot passed two matching GET-only audits across 73 repositories with 1,898 results, 2,045 coverage records, 669 findings, minimal detail, and zero writes. Credential and plan-availability diagnostics exposed no repository or credential details.
